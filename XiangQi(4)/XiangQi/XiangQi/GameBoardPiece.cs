...
namespace XiangQi
{
    ...
    class Advisor : Piece
    {
        ...
        public void ValidMovement(MovePoint[] movePoints)//最终判定可以去的地方
        {
            ...
            {
                if (movePoints[i].horizontal >= 3 && movePoints[i].horizontal <= 5)//判定是否在城内 
                {
                    if (movePoints[i].side == Player_side.red)
                    {
                        if (movePoints[i].vertical >= 7 && movePoints[i].vertical <= 9)//判定红色方要走的地方是否在红色方城内
                        {
                    ...
                    {
                        if (movePoints[i].vertical >= 0 && movePoints[i].vertical <= 3)//判定黑色方要走的地方是否在黑色方城内
                        {
                            movePoints[i].CanReach = true;//可以去的格子就给个true
                        }
                    }
